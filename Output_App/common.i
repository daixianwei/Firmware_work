#line 1 "Management\\Common_CRC\\Common.c"
...
#line 8 "Management\\Common_CRC\\Common.c"

 
 
#line 17 "Management\\Common_CRC\\Common.c"
...
static uint16 Calc4BitsCRC(uint8 value, uint16 remainder)
{
    ...
    tableIndex = ((value) & (0x0F)) ^ ((remainder) >> ((sizeof(uint16) * 8)
    				- (0x04)));
    remainder = (crcTable)[tableIndex] ^ (remainder << (0x04));
    return remainder;
}
		...
		uint16 remainder, uint16 xorMask)
{
    ...
    for(messageIndex = 0; messageIndex < length; messageIndex++)
    {
        ...
        remainder = Calc4BitsCRC(byteValue>>(0x04), remainder);
