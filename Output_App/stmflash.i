#line 1 "USB\\USB_Communication\\stmflash.c"
...
#line 8 "USB\\USB_Communication\\stmflash.c"
...
void STMFlash_Write(uint32 WriteAddr, uint16 *pBuffer, uint16 NumToWrite)
{
	...
	if(WriteAddr < ((uint32_t)0x08000000) ||
			(WriteAddr >= (((uint32_t)0x08000000) + ((512) << 10))))
		return;

	FLASH_Unlock();
	offaddr = WriteAddr - ((uint32_t)0x08000000);
	secpos = offaddr / (2048);
	secoff = (offaddr % (2048)) >> 1;
	secremain = ((2048) >> 1) - secoff;
	...
	while(1) 
	{	
		STMFlash_Read(secpos * (2048) + ((uint32_t)0x08000000), flashTmpBuffer,
				(2048) >> 1);
		...
		if(i < secremain)
		{
			FLASH_ErasePage(secpos * (2048) + ((uint32_t)0x08000000));
			for(i = 0; i < secremain; i++)
			{
				flashTmpBuffer[i + secoff] = pBuffer[i];
			}

			STMFlash_Write_NoCheck(secpos * (2048) + ((uint32_t)0x08000000),
					flashTmpBuffer, (2048) >> 1);
		}else
